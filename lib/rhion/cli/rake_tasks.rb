...
# Loads the Rhion applications mounted within the project.
# Setting up the required environment for Rhion.
task :environment do
...
# Loads skeleton Rhion environment, no models, no application settings.
task :skeleton do
  module Rhion::Reloader
...
namespace :routes do
  task :app, [:app] => :environment do |t, args|
    app = Rhion.mounted_apps.find { |app| app.app_class == args.app }
