# Defines the log level for a Rhino project.
RHINO_LOG_LEVEL = ENV['RHINO_LOG_LEVEL'] unless defined?(RHINO_LOG_LEVEL)

# Defines the logger used for a Rhino project.
RHINO_LOGGER = ENV['RHINO_LOGGER'] unless defined?(RHINO_LOGGER)
...
module Rhino
  ...
  #   require 'logger'
  #   Rhino.logger = Logger.new(STDOUT)
  #
  # @example using ActiveSupport
  #   require 'active_support/buffered_logger'
  #   Rhino.logger = Buffered.new(STDOUT)
  #
  def self.logger=(value)
    Rhino::Logger.logger = value
  end

  ##
  # Rhinos internal logger, using all of Rhino log extensions.
  #
  class Logger
    ##
    ...
    module Extensions
      ##
      # Generate the logging methods for {Rhino.logger} for each log level.
      #
      Rhino::Logger::Levels.each_pair do |name, number|
      ...
      def push(message = nil, level = nil)
        add(Rhino::Logger::Levels[level], format(message, level))
      end

      ##
      # Formats the log message. This method is a noop and should be implemented by other
      # logger components such as {Rhino::Logger}.
      #
      # @param [String] message
      ...
      # @example
      #   Rhino.logger = Logger.new($stdout)
      #   Rhino.logger.colorize!
      #   Rhino.logger.debug("Fancy Rhino debug string")
      def colorize!
      ...
      #
      # @example
      #   Rhino.logger.exception e
      #   Rhino.logger.exception(e, :short)
    ...
    module Colorize
      ...
      # @param [String, Symbol] level
      #
      # @see Rhino::Logging::ColorizedLogger::ColoredLevels
      #
      def colorize(string, *colors)
      ...
      def stylized_level(level)
        ...
        style = ColoredLevels[level][1]
        ['Rhino '.colorize(style), super, ''] * ''
      end
    end
    ...
    #
    # @example
    #   Rhino::Logger::Config[:development] = { :log_level => :debug, :stream => :to_file }
    #   # or you can edit our defaults
    #   Rhino::Logger::Config[:development][:log_level] = :error
    #   # or you can use your stream
    #   Rhino::Logger::Config[:development][:stream] = StringIO.new
    #
    # Defaults are:
    #
    ...
    # You can do so by setting RHINO_LOGGER:
    #
    #   RHINO_LOGGER = { :staging => { :log_level => :debug, :stream => :to_file }}
    #
    Config = {
      :production  => { :log_level => :warn,  :stream => :to_file },
    ...
    Config.merge!(RHINO_LOGGER) if RHINO_LOGGER
    ...
    def self.logger=(logger)
      logger.extend(Rhino::Logger::Extensions)
    ...
    #
    # @return [Rhino::Logger]
    #   A {Rhino::Logger} instance
    #
    def self.setup!
      self.logger = begin
        config_level = (RHINO_LOG_LEVEL || Rhino.env || :test).to_sym # need this for RHINO_LOG_LEVEL
        ...
        stream = case config[:stream]
          when :to_file
            FileUtils.mkdir_p(Rhino.root('log')) unless File.exist?(Rhino.root('log'))
            File.new(Rhino.root('log', "#{Rhino.env}.log"), 'a+')
        ...
        Rhino::Logger.new(config.merge(:stream => stream))
      end
    end
    ...
    def initialize(options={})
      ...
      @level           = options[:log_level] ? Rhino::Logger::Levels[options[:log_level]] : Rhino::Logger::Levels[:debug]
    ...
    def format(message, level)
      @format_message % [stylized_level(level), Time.now.strftime(@format_datetime).colorize(:yellow), message.to_s.strip]
      #% [stylized_level(level), colorize(Time.now.strftime(@format_datetime), :yellow), message.to_s.strip]
    end

    ##
    # Rhino::Logger::Rack forwards every request to an +app+ given, and
